...
def build_cover_sheet(wb):
    ...
    for sheet_name, label in sheets_to_link:
        ...
        cell.font = Font(size=11, color=COLORS['accent_teal'], underline='single')
        nav_row += 1
...
def build_dashboard_sheet(wb):
    ...
    kpi_data = [
        ...
        ('Win Rate %', '=IFERROR(COUNTIF(Tbl_Journal[Net P&L],">0")/COUNTA(Tbl_Journal[Net P&L])*100,0)', COLORS['accent_teal']),
        ('Profit Factor', '=IFERROR(SUMIF(Tbl_Journal[Net P&L],">0")/ABS(SUMIF(Tbl_Journal[Net P&L],"<0")),0)', COLORS['accent_gold']),
        ('Expectancy (R)', '=IFERROR(AVERAGE(Tbl_Journal[R-Multiple]),0)', COLORS['accent_teal']),
        ...
        ('Best Trade', '=IFERROR(MAX(Tbl_Journal[Net P&L]),0)', COLORS['accent_teal']),
...
def build_profit_dashboard_sheet(wb):
    ...
    kpis = [
        ('Win Rate %', '=IFERROR(COUNTIF(Tbl_Journal[Net P&L],">0")/COUNTA(Tbl_Journal[Net P&L])*100,0)', COLORS['accent_teal']),
        ('Profit Factor', '=IFERROR(SUMIF(Tbl_Journal[Net P&L],">0")/ABS(SUMIF(Tbl_Journal[Net P&L],"<0")),0)', COLORS['accent_gold']),
        ('Avg Win $', '=IFERROR(AVERAGEIF(Tbl_Journal[Net P&L],">0"),0)', COLORS['accent_teal']),
        ...
        ('Largest Win', '=IFERROR(MAX(Tbl_Journal[Net P&L]),0)', COLORS['accent_teal']),
        ...
        ('Total Trades', '=COUNTA(Tbl_Journal[Trade #])', COLORS['accent_gold']),
        ('Total R Gained', '=IFERROR(SUM(Tbl_Journal[R-Multiple]),0)', COLORS['accent_teal']),
    ]
...
def build_equity_curve_sheet(wb):
    ...
    apply_style(ws['A5'], StylePresets.kpi_value(COLORS['accent_teal']))
...
def generate_readme():
    """Generate README_HowToUse.txt file."""
    readme_content = """
╔══════════════════════════════════════════════════════════════════╗
